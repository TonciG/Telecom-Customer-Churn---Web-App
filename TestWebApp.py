# This web app (made to be run on Streamlit) builds on the ideas from Test2.py and Test3.py.
# It uses previously trained Orange model (comes in .pkcls format) to make predictions on
# data provided by the user via the Streamlit-built web app. The .pkcls model has all the
# model parameters, model hyperparameters, and the entire pre-processing pipeline (with all
# the necessary pre-processing techniques and their training-data-estimated parameters), hence 
# it is ready to be used for predictions. This means that the data for prediction DOES NOT need
# to be previously pre-processed as this will be done automatically by the .pkcls model. However,
# for this to work, the user provided input data need to be in Orange.data.Table format. This
# format holds BOTH the domain (names of input features, their types, and, if categorical, the
# value names) and input values. Once in this format, to make a prediction with the .pkcls model,
# use: loaded_model(data_input).
...
# st.container() = horizontal section
# st.columns() = vertical sections
# Use "with container_name/column_name:" to add any content to either
# container or column
...
# Stack contains Pre-processing pipeline (made on the training dataset) +
...
def get_user_input():
    ...
    # min and max values are based on ExistingCustomers.csv
    ...
    # in this format, the data is now ready to be fed to the Orange model
...
with modelPrediction_cont:
    ...
    # loaded_model(df_userinput) returns the index of predicted class. In case of
    # Telecom Customer Churn dataset: 0. is no; 1. is yes.
    
    # class_value = int(loaded_model(df_userinput))
    # classes = ["no", "yes"]
    # print("This customer will churn: ", classes[class_value])
